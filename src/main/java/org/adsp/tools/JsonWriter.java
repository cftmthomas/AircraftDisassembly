...
public class JsonWriter {
    ...
    public static void writeInstanceToPath(Instance instance, String path){
        writeInstanceToFile(instance, new File(path));
    }

    public static void writeInstanceToFile(Instance instance, File file){
        try{
            File out = file.isDirectory() ? new File(file.getAbsolutePath() + "/" + instance.name() + ".json") : file;
            out.getParentFile().mkdirs();
            mapper.writerWithDefaultPrettyPrinter().writeValue(out, instance);
        } catch (IOException e) {
